...
const speechTreeReducer = (state = initialState, action) => {
  ...
  case types.REMOVE_FEATURE:
    return  {
      ...
      events: state.contexts.filter(ev => ev.feId !== action.payload.id),
  ...
  case types.REMOVE_CONTEXT:
    return  {
      ...
      events: state.contexts.filter(ev => ev.coId !== action.payload.id),
  ...
  case types.REMOVE_EVENT:
    return  {
      ...state,
      events: state.contexts.filter(ev => ev.id !== action.payload.id),
